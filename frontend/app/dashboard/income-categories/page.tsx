...
export default function IncomeCategoriesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await incomeCategoriesApi.create({ name, description })
      setName("")
      setDescription("")
      ...
      toast({
        title: "Failed to add income category",
  ...
  const handleUpdate = async (id: number) => {
    setIsLoading(true)
    try {
      await incomeCategoriesApi.update(id, { name: editName, description: editDescription })
      toast({ title: "Income category updated successfully!" })
      setEditId(null)
      fetchCategories()
    } catch (error: any) {
      toast({
        title: "Failed to update income category",
        description: error?.message || "Something went wrong.",
        variant: "destructive",
      })
    } finally {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast({
        title: "Failed to delete income category",
        description: error?.message || "Something went wrong.",
        variant: "destructive",
      })
    } finally {
    ...
    <div className="space-y-6">
      <Card className="max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="pt-6">
            <h3 className="font-semibold mb-2">Existing Income Categories</h3>
